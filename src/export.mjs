...
export async function exportTest({ scenario, outDir, mode, dir, entry, url, isolation, viewport, device, port = 4173, overwrite = false }) {
    ...
    await write("playwright.config.ts", renderConfig({ mode, dir, port, baseURL, gamePath, isolation, viewport, device, usesTouch }));
    if (mode === "dir") {
...
function renderConfig({ mode, dir, port, baseURL, gamePath, isolation, viewport, device, usesTouch }) {
    const use = [];
