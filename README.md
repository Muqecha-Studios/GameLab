...
| Shell (browser tab) | The served page at `/__gp/`: the game in an iframe with FPS badge, a **device profile** menu (resolution + DPR/UA/touch/cores emulation, editable), isolation toggle, a Console drawer and a **Perf** tab — frame percentiles + 1 %/0.1 % lows + jitter, **main-thread vs GPU time per frame with a bound verdict** (CPU-bound / GPU-bound / vsync-limited + headroom), hitches and long tasks **with their cause** (shader compile, texture upload, wasm grow, GC, game event), render resolution vs display, WebGL counters (draws, program switches, binds, state changes, live texture/buffer memory), JS heap / wasm memory / GC / DOM, input→frame latency, audio latency, threads, load timeline, plain-English **Findings** that name the bottleneck and the fix, a ● Profile button that lists the hottest functions with file:line, and a **Timeline** tab — FPS, frame/main-thread/GPU ms, input lag, draw calls, GL state, memory and engine timings graphed over time (30 s → 20 min) with game events, console errors and reloads as markers; hover for values, toggle series, export CSV |
